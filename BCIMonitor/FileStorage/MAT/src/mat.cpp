...
{
    size_t dim[2]={1,1};
    const char *field[10]={"chanlocs","data","heart","srate","filename","event","urevent","ref","age","set"};
    eeg_struct=Mat_VarCreateStruct("EEG",2,dim,field,10);
}
...
void MAT::save(double *data, quint32 size, double *heart, quint32 heart_num)
{
    double* Matdata=data;
    uint row=16;
    uint col=size;
    ...
    dims[0]=3;
    dims[1]=heart_num;
    matvar_t *heart_data_field=Mat_VarCreate(NULL,MAT_C_DOUBLE,MAT_T_DOUBLE,2,dims,heart,0);
    Mat_VarSetStructFieldByName(eeg_struct,"heart",0,heart_data_field);
